(function ($) {
   ...
   */
  Backdrop.ajax_facets = {};
  // Store current query value.
  Backdrop.ajax_facets.queryState = null;
  // State of each facet.
  Backdrop.ajax_facets.facetQueryState = null;
  // Determine if it is first page load. Will be reset in Backdrop.ajax_facets.initHistoryState.
  Backdrop.ajax_facets.firstLoad = true;
  // Settings of each ajax facet.
  Backdrop.ajax_facets.facetsSettings = {};
  // Buttons for ajax facets.
  Backdrop.ajax_facets.ajax_facets_buttons = false;
  // Force update of the results
  Backdrop.ajax_facets.force_update_results = false;
  // Tooltip timeout handler.
  Backdrop.ajax_facets.tooltipTimeout;
  // Name of the facet which was clicked.
  Backdrop.ajax_facets.current_facet_name = undefined;

  // You can use it for freeze facet form elements while ajax is processing.
  Backdrop.ajax_facets.beforeAjaxCallbacks = {};

  // You can use it to make actions after the ajax has updated the content.
  Backdrop.ajax_facets.afterContentUpdateCallbacks = {};

  Backdrop.ajax_facets.beforeAjax = function () {
    $.each(Backdrop.ajax_facets.beforeAjaxCallbacks, function () {
      if ($.isFunction(this)) {
        this();
      }
    });
  ...
  Backdrop.ajax_facets.afterContentUpdate = function (ajax, response) {
    Backdrop.ajax_facets.force_update_results = false;
    $.each(Backdrop.ajax_facets.afterContentUpdateCallbacks, function () {
      if ($.isFunction(this)) {
        this(ajax, response);
      }
    });
  ...
  Backdrop.behaviors.ajax_facets = {
    attach: function (context, settings) {
      if (!Backdrop.ajax_facets.queryState) {
        // Add property for views and set data about facet values.
        Backdrop.ajax_facets.queryState = {
          'views': new Array(),
        ...
        if (settings.facetapi.searchPath) {
          Backdrop.ajax_facets.queryState['searchPath'] = settings.facetapi.searchPath;
        }
        if (settings.facetapi.index_id) {
          Backdrop.ajax_facets.queryState['index_id'] = settings.facetapi.index_id;
        }
        // Check view name and display name for all views.
        if (settings.facetapi.views) {
          $.each(settings.facetapi.views, function(key, view) {
            Backdrop.ajax_facets.queryState['views'][key] = {};
            // Get view dom id.
            var viewDomId = Backdrop.ajax_facets.getViewDomId(settings, key);
            if (viewDomId) {
              // Take settings from view if it works by AJAX.
              if (Backdrop.views && Backdrop.views.instances['views_dom_id:' + viewDomId]) {
                $.extend(Backdrop.ajax_facets.queryState['views'][key], Backdrop.views.instances['views_dom_id:' + viewDomId].settings);
              }
              // If this view doesn't use ajax.
                ...
                if (settings.facetapi.view_args[nameDisplay]) {
                  Backdrop.ajax_facets.queryState['views'][key]['view_args'] = settings.facetapi.view_args[nameDisplay];
                }
                // Respect view path.
                if (settings.facetapi.view_path[nameDisplay]) {
                  Backdrop.ajax_facets.queryState['views'][key]['view_path'] = settings.facetapi.view_path[nameDisplay];
                }
              }
            }
            ...
            if (view.view_name && view.view_display_id) {
              ...
              if (settings.facetapi.exposed_input[nameDisplay]) {
                $.extend(Backdrop.ajax_facets.queryState, settings.facetapi.exposed_input[nameDisplay]);
              }
            }
          });
        }
        ...
        if (settings.facetapi.facet_field) {
          Backdrop.ajax_facets.queryState['facet_field'] = settings.facetapi.facet_field;
        }
      }
      ...
      if (settings.facetapi) {
        for (var index in settings.facetapi.facets) {
          Backdrop.ajax_facets.bindResetLink(settings.facetapi.facets[index].id, index, settings);
          ...
          if (settings.facetapi.facets[index].widget == 'facetapi_ajax_checkboxes') {
            $('#' + settings.facetapi.facets[index].id + '-wrapper input.facet-multiselect-checkbox:not(.processed)').change(
              ...
              Backdrop.ajax_facets.processCheckboxes
            ).addClass('processed');
          }
          ...
          if (settings.facetapi.facets[index].widget == 'facetapi_ajax_select') {
            $('#' + settings.facetapi.facets[index].id + '-wrapper select:not(.processed)').each(function () {
              $(this).change(
                [settings.facetapi.facets[index]],
                Backdrop.ajax_facets.processSelectbox
              ).addClass('processed');
            });
          }
          ...
          if (settings.facetapi.facets[index].widget == 'facetapi_ajax_links') {
            $('#' + settings.facetapi.facets[index].id + '-wrapper a:not(.processed)').each(function () {
              $(this).click(
                [settings.facetapi.facets[index]],
                Backdrop.ajax_facets.processLink
              ).addClass('processed');
            });
          }
            ...
            $('#' + settings.facetapi.facets[index].id + '-wrapper div.slider-wrapper:not(.processed)').each(function () {
              ...
              $sliderWrapper.slider({
                ...
                stop: function( event, ui ) {
                  Backdrop.ajax_facets.processSlider($sliderWrapper, ui.values[0], ui.values[1]);
                }
              }).addClass('processed');
              ...
              $sliderWrapperParent.find("input[type='text']").each(function() {
                $(this).change(function() {
                  ...
                  if (isFinite(min) && isFinite(max) && min < max) {
                    ...
                    Backdrop.ajax_facets.processSlider($sliderWrapper, min, max);
                  }
                });
              });
          ...
          if (null != settings.facetapi.facets[index].limit) {
            ...
            if (Backdrop.facetapi) {
              Backdrop.facetapi.applyLimit(settings.facetapi.facets[index]);
            }
          }

          // Save settings for each facet by name.
          Backdrop.ajax_facets.facetsSettings[settings.facetapi.facets[index].facetName] = settings.facetapi.facets[index];
        }
      }
  ...
  Backdrop.behaviors.ajax_facets_block = {
    attach: function (context, settings) {
      // Reset all link.
      $('.ajax-facets-reset-all-link:not(.processed)').bind('click', function (e) {
        ...
        Backdrop.ajax_facets.queryState.f = [];

        // Send ajax query with first found facet.
        Backdrop.ajax_facets.force_update_results = true;
        Backdrop.ajax_facets.sendAjaxQuery({
      ...
      if ($submitLink.length) {
        // Submit link found.
        Backdrop.ajax_facets.ajax_facets_buttons = true;
        // Submit all link.
  ...
  Backdrop.ajax_facets.bindResetLink = function (facetWrapperId, index, settings) {
    var facet_values = Backdrop.ajax_facets.getFacetValues();
    ...
    $facetWrapper.find('a.reset-link:not(".processed")').addClass('processed').click(function (event) {
      ...
      Backdrop.ajax_facets.excludeCurrentFacet(facetName);
      Backdrop.ajax_facets.sendAjaxQuery({
        pushStateNeeded: true,
        searchResultsNeeded: true
      });
   ...
   */
  Backdrop.ajax_facets.processSelectbox = function (event) {
    var $this = $(this),
      facetName = $this.data('facet-name');
    // Init history.
    Backdrop.ajax_facets.initHistoryState($this);
    // If facets are already defined in queryState.
    if (Backdrop.ajax_facets.queryState['f']) {
      // Exclude all values for this facet from query.
      Backdrop.ajax_facets.excludeCurrentFacet(facetName);

      /* Default value. */
      if ($this.find(":selected").val() == '_none') {
        delete Backdrop.ajax_facets.queryState['f'][Backdrop.ajax_facets.queryState['f'].length];
      } else {
        Backdrop.ajax_facets.queryState['f'][Backdrop.ajax_facets.queryState['f'].length] = facetName + ':' + $this.find(":selected").val();
      }
    }

    Backdrop.ajax_facets.sendAjaxQuery({
      pushStateNeeded: !Backdrop.ajax_facets.ajax_facets_buttons,
      searchResultsNeeded: !Backdrop.ajax_facets.ajax_facets_buttons
    }, $this);
  };
   ...
   */
  Backdrop.ajax_facets.processCheckboxes = function (event) {
    var $this = $(this),
      facetName = $this.data('facet-name'),
    ...
    if (Backdrop.ajax_facets.queryState['f']) {
      ...
      if ($this.is(':checked')) {
        ...
        for (var index in Backdrop.ajax_facets.queryState['f']) {
          if (Backdrop.ajax_facets.queryState['f'].hasOwnProperty(index)) {
            // If we already have this value in queryState.
            if (Backdrop.ajax_facets.queryState['f'][index] == name_value) {
        ...
        if (addCurrentParam) {
          ...
          if (Backdrop.ajax_facets.facetsSettings[rawFacetName].limit_active_items) {
            Backdrop.ajax_facets.excludeCurrentFacet(facetName);
          }
          Backdrop.ajax_facets.queryState['f'][Backdrop.ajax_facets.queryState['f'].length] = name_value;
        }
      }
      // If we unset filter, remove it from query.
        ...
        for (var index in Backdrop.ajax_facets.queryState['f']) {
          if (Backdrop.ajax_facets.queryState['f'].hasOwnProperty(index)) {
            if (Backdrop.ajax_facets.queryState['f'][index] != name_value) {
              queryNew[queryNew.length] = Backdrop.ajax_facets.queryState['f'][index];
            }
          }
        }
        Backdrop.ajax_facets.queryState['f'] = queryNew;
      }
    }
   ...
   */
  Backdrop.ajax_facets.processLink = function (event) {
    var $this = $(this),
      facetName = $this.data('facet-name'),
      name_value = facetName + ':' + $this.attr('data-facet-value'),// $.data can round decimal values like 4.0 to 4, avoid it.
      rawFacetName = $this.data('raw-facet-name');
   ...
   */
  Backdrop.ajax_facets.processSlider = function($sliderWrapper, min, max) {
    var $this = $(this),
      facetName = $sliderWrapper.data('facet-name');
    // Init history.
    Backdrop.ajax_facets.initHistoryState($sliderWrapper);
    // If facets are already defined in queryState.
    if (Backdrop.ajax_facets.queryState['f']) {
      // Exclude all values for this facet from query.
      Backdrop.ajax_facets.excludeCurrentFacet(facetName);
      Backdrop.ajax_facets.queryState['f'][Backdrop.ajax_facets.queryState['f'].length] = facetName + ':[' + min + ' TO ' + max + ']';
    }
   ...
   * Exclude all values for this facet from query.
   */
  Backdrop.ajax_facets.excludeCurrentFacet = function (facetName) {
    facetName = facetName + ':';
    var queryNew = [];
    for (var index in Backdrop.ajax_facets.queryState['f']) {
      if (Backdrop.ajax_facets.queryState['f'].hasOwnProperty(index)) {
        if (Backdrop.ajax_facets.queryState['f'][index].substring(0, facetName.length) != facetName) {
          queryNew[queryNew.length] = Backdrop.ajax_facets.queryState['f'][index];
        }
      }
    }
   ...
   * Send ajax.
   */
  Backdrop.ajax_facets.sendAjaxQuery = function (options, $facet) {
    Backdrop.ajax_facets.beforeAjax();
    var data = Backdrop.ajax_facets.queryState;
    // Render the exposed filter data to send along with the ajax request
    $.each(Backdrop.ajax_facets.queryState.views, function (key, view) {
    ...
    Backdrop.ajax_facets.current_facet_name = $facet ? $facet.data('raw-facet-name') : undefined;

    // Do not auto update facets and counts if auto update is turned off in block settings.
    if (!Backdrop.settings.ajax_facets.auto_update_facets && !options.searchResultsNeeded && !options.pushStateNeeded) {
      return;
    }

    // Notify Backdrop do we need search results or not.
    data.searchResultsNeeded = options.searchResultsNeeded;

    var settings = {
      url: encodeURI(Backdrop.settings.basePath + Backdrop.settings.pathPrefix + 'ajax/ajax_facets/refresh'),
      submit: {'ajax_facets': data}
    };
    var ajax = new Backdrop.ajax(false, false, settings);
    ajax.success = function (response, status) {
      // Push new state only on successful ajax response.
      if (options.pushStateNeeded) {
        Backdrop.ajax_facets.writeState(false);
      }
  ...
  Backdrop.ajax_facets.getFacetValues = function () {
    var f = Backdrop.ajax_facets.queryState.f;
    var facets_values = {};
    var symbol = ':';
  ...
  Backdrop.ajax_facets.getViewDomId = function(settings, key) {
   ...
   */
  Backdrop.ajax_facets.getFacetsQueryUrl = function (baseUrl) {
    var query = {
      'f': []
    };

    // Clone variable.
    $.extend(true, query.f, Backdrop.ajax_facets.queryState.f);
    ...
    if (Backdrop.ajax_facets.queryState.query) {
      query.query = Backdrop.ajax_facets.queryState.query;
    }

    if (Backdrop.ajax_facets.queryState.order) {
      query.order = Backdrop.ajax_facets.queryState.order;
      query.sort = Backdrop.ajax_facets.queryState.sort;
    }

    if (Backdrop.ajax_facets.queryState.pages) {
      query.pages = Backdrop.ajax_facets.queryState.pages;
    }

    // Respect existing query parameters in url.
    // Merge respected parameters with facet parameters recursively without duplicates.
    $.extend(true, query, Backdrop.ajax_facets.simplifyObject(Backdrop.ajax_facets.getAdditionalQueryParameters()));
   ...
   */
  Backdrop.ajax_facets.getAdditionalQueryParameters = function () {
    var result = {};
   ...
   */
  Backdrop.ajax_facets.simplifyObject = function (obj) {
    for (var name in obj) {
   ...
   * handler.
   */
  Backdrop.ajax_facets.initHistoryState = function ($facet) {
    // Set the initial state only initial page load.
    if (Backdrop.ajax_facets.firstLoad) {
      Backdrop.ajax_facets.firstLoad = false;
      Backdrop.ajax_facets.writeState(true);
    }
  };
   ...
   */
  Backdrop.ajax_facets.writeState = function (replace) {
    var stateUrl = null,
      state = {
        facets: Backdrop.ajax_facets.queryState['f']
      },
      title = document.title,
      method = 'replaceState';

    if (replace !== true) {
      stateUrl = Backdrop.ajax_facets.getFacetsQueryUrl(Backdrop.settings.facetapi.searchUrl);
      method = 'pushState';
    }
    ...
    if (history[method]) {
      ...
      if (Backdrop.settings.facetapi.isHistoryJsExists) {
        var $window = $(window);

        $window.unbind('statechange', Backdrop.ajax_facets.reactOnStateChange);
        History[method](state, title, stateUrl);
        $window.bind('statechange', Backdrop.ajax_facets.reactOnStateChange);
      }
    }
  };
   ...
   */
  Backdrop.ajax_facets.reactOnStateChange = function () {
    var state = null,
      facets = [];
    ...
    if (window.location.pathname === Backdrop.settings.facetapi.searchUrl && facets) {
      Backdrop.ajax_facets.queryState['f'] = facets;
      Backdrop.ajax_facets.force_update_results = true;
      Backdrop.ajax_facets.sendAjaxQuery({
  ...
  // All farther work does Backdrop.ajax_facets.pushState() function.
  // Try to use HTML5 history object.
  if (history) {
    window.onpopstate = function () {
      Backdrop.ajax_facets.reactOnStateChange();
    };
  } else {
    // If history.js Adapter available - use it to bind "statechange" event.
    if (History.Adapter && Backdrop.ajax_facets.firstLoad) {
      History.Adapter.bind(window, 'statechange', Backdrop.ajax_facets.reactOnStateChange);
    }
  }

  /* Show tooltip if facet results are not updated by ajax (in settings). */
  Backdrop.ajax_facets.showTooltip = function ($, response) {
    // Reset the timeout handler to avoid troubles when user is clicking on items very fast.
    window.clearTimeout(Backdrop.ajax_facets.tooltipTimeout);

    // If we have chosen facet.
    if (Backdrop.ajax_facets.current_facet_name) {
      var pos = $('[data-raw-facet-name=' + Backdrop.ajax_facets.current_facet_name + ']').offset(),
        $tooltip = $('#ajax-facets-tooltip');
      $tooltip.css('top', pos.top - 15);
      $tooltip.css('left', pos.left - $tooltip.width() - 40);
      $tooltip.show();
      $tooltip.find('span').html(Backdrop.t('Found:') + ' ' + response.total_results);

      Backdrop.ajax_facets.tooltipTimeout = setTimeout(function () {
        $tooltip.hide(250);
      }, 3000);
    }
  };
  ...
  if (Backdrop.ajax) {
    ...
    Backdrop.ajax.prototype.commands.ajax_facets_update_content = function(ajax, response) {
      if (response.data.activeItems) {
        Backdrop.ajax_facets.facetQueryState = response.data.activeItems;
      }
      // After Ajax success we should update reset, apply link to handle proper redirects.
      if (response.data.resetUrls && Backdrop.settings.facetapi.facets) {
        for (var index in Backdrop.settings.facetapi.facets) {
          if (response.data.resetUrls[Backdrop.settings.facetapi.facets[index].facetName]) {
            // Update path from response.
            Backdrop.settings.facetapi.facets[index].resetPath = response.data.resetUrls[Backdrop.settings.facetapi.facets[index].facetName];
          }
        }
      }
      ...
      var show_tip = false;
      var buttons_exist = Backdrop.ajax_facets.ajax_facets_buttons,
        force_update_results = Backdrop.ajax_facets.force_update_results;
      ...
      if (response.data.settings.views) {
        Backdrop.settings.views = response.data.settings.views;
      }

      // We add this here so we can run custom code after content updates
      Backdrop.ajax_facets.afterContentUpdate(ajax, response);

      Backdrop.attachBehaviors();
      // Show the tooltip if need.
      if (show_tip) {
        Backdrop.ajax_facets.showTooltip($, response.data);
      }
    };
  }
})(jQuery);